...
def get_video_extension():
    print('请输入转换前文件类型(eg .mp4)')
    extension_in = input()
    print('请输入转换后文件类型(eg .flv)')
    extension_out = input()
...
def convert(file_name, convert_ext):
    cmd_str = f'ffmpeg -i "{file_name}" -c:v copy -c:a copy "{file_name[:-4]+convert_ext}"'
    print(cmd_str)
...
def run():
    ...
    extension_in, extension_out = get_video_extension()
